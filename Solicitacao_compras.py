...
# =========================
# APP + PATHS (CORREÇÃO PARA RENDER)
# =========================
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
# Forçamos o caminho absoluto para evitar erro de templates não encontrados
app = Flask(__name__, 
            template_folder=os.path.join(BASE_DIR, "templates"),
            static_folder=os.path.join(BASE_DIR, "static"))

app.config["SECRET_KEY"] = os.getenv("FLASK_SECRET", "dev_secret_change_me")
app.config["SQLALCHEMY_DATABASE_URI"] = os.getenv("DATABASE_URL", "sqlite:///" + os.path.join(BASE_DIR, "instance", "app.db"))
...
# =========================
# ROTAS - TODAS AS PÁGINAS DA SUA IMAGEM
# =========================

@app.route("/")
def home():
    products = Product.query.order_by(Product.created_at.desc()).all()
    # index.html deve conter {% include 'orcamento_modal.html' %}
    return render_template("index.html", products=products, year=datetime.now().year)

@app.route("/admin/login", methods=["GET", "POST"])
def admin_login():
    if request.method == "POST":
        # Credenciais via env vars no Render
...
def admin_edit_product(id):
    ...
    return render_template("admin_edit_product.html", product=product, year=datetime.now().year)
